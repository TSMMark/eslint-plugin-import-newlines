...
const fixer = (node, semi, spacer = '\n') => (eslintFixer) => {
  ...
  node.specifiers.forEach((currentNode) => {
    const specifierIsType = currentNode.importKind === 'type';
    switch (currentNode.type) {
      case SPEC_DEFAULT_IMPORT:
        defaultImport = applyAlias(currentNode);
        break;
      case SPEC_IMPORT:
        objectImports.push(`${specifierIsType ? 'type ' : ''}${applyAlias(currentNode)}`);
        break;
      default:
        break;
    }
  ...
  const objectImportsValue = objectImports.length > 0
    ...
    : '';
  const newValue = `import ${isType ? 'type ' : ''}${defaultImportValue}${objectImportsValue} from ${node.source.raw}${semi ? ';' : ''}`;
